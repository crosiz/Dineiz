...
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Billing', href: '/subscriptions/billing', icon: Receipt },
    ],
  },
  {
    ...
    items: [
      ...
      { label: 'Email Log', href: '/communications/email-log', icon: Mail },
    ],
  },
  {
    title: 'SYSTEM',
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-surface-base text-slate-900">
      {/* Mobile Backdrop */}
      {sidebarOpen && (
        <div
      ...
      {/* Sidebar Component — dark rail, mirrors the tenant dashboard's sidebar treatment */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-50 w-[240px] bg-[#101012] border-r border-zinc-800/80 flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 ${
      ...
      >
        ...
        <div className="h-16 px-5 flex items-center justify-between border-b border-zinc-800/80">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <div>
              ...
              <span className="text-xs font-semibold text-orange-400 block -mt-1 uppercase tracking-wider">Admin</span>
            </div>
          </Link>
        ...
        {/* Navigation Items */}
        <div className="flex-1 overflow-y-auto py-4 px-3 space-y-6 sidebar-scrollbar">
        ...
        <div className="p-3 border-t border-zinc-800/80 bg-zinc-950/40">
          <div className="p-2.5 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-between">
            <div className="min-w-0">
              <p className="text-xs font-bold text-zinc-200 truncate">{user?.name || 'Super Admin'}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:pl-[240px]">
        {/* Top Bar */}
        <header className="h-16 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-slate-900 tracking-wide">Dineiz Admin</span>
              <span className="text-[10px] bg-orange-50 text-orange-700 border border-orange-200 px-2 py-0.5 rounded-full font-semibold">
                INTERNAL OS
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                <span className="block text-xs font-semibold text-slate-900">{user?.name || 'Super Admin'}</span>
                <span className="block text-[10px] text-slate-500">{user?.email}</span>
              </div>

              <button
                onClick={handleSignOut}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-rose-50 hover:text-rose-600 text-slate-600 border border-slate-200 text-xs font-semibold transition-all"
              >
